...
export const CompanyDissolutionOfEirl = {
  async getById(company_id) {
    const [rows] = await db.query(
      'SELECT id, contact_email, contact_name, contact_phone FROM company_dissolution_of_eirl_request WHERE company_id = ?',
      [company_id]
    )
    return rows[0]
  },
  ...
  async create(company_id, contact_name, contact_email, contact_phone) {
    const [result] = await db.query(
      `INSERT INTO company_dissolution_of_eirl_request 
  ...
  async update(company_id, contact_name, contact_email, contact_phone) {
    await db.query(
      `UPDATE company_dissolution_of_eirl_request 
