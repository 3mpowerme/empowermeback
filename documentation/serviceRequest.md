...
- Source route file: `src/routes/serviceRequest.routes.js`
- Mounted prefixes: `/api/service-request`

## GET /api/service-request/

### Description
- Handler: `ServiceRequestController.getAll`
...
### Example Request
```http
GET /api/service-request/ HTTP/1.1
Host: <api-host>
```
...
### Description
- Handler: `ServiceRequestController.updateStatus`
- Observed behavior: route delegates logic to controller/service/model layers.
...
### Validations
- Joi middleware detected: `validate(updateServiceRequestStatusSchema)`
...
### Example Request
```http
PUT /api/service-request/status/:id HTTP/1.1
Host: <api-host>
```
...
### Validations
- Joi middleware detected: `validate(serviceRequestSchema)`
...
- `company_tax_address`: `Joi.string().max(255)`
- `company_tax_key`: `Joi.string().max(255)`
...
- `contact_phone`: `Joi.string().pattern(/^[0-9]{7`
