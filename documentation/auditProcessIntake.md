# auditProcessIntake API

- Source route file: `src/routes/auditProcessIntake.routes.js`
- Mounted prefixes: `/api/company-audit-request`

## GET /api/company-audit-request/:companyId

### Description
- Handler: `AuditProcessIntakeController.getAll`
...
### Example Request
```http
GET /api/company-audit-request/:companyId HTTP/1.1
Host: <api-host>
```
...
### Description
- Handler: `AuditProcessIntakeController.create`
- Observed behavior: route delegates logic to controller/service/model layers.
...
### Validations
- Joi middleware detected: `validate(createAuditProcessIntakeSchema)`
...
### Example Request
```http
POST /api/company-audit-request/:companyId HTTP/1.1
Host: <api-host>
```
...
### Description
- Handler: `AuditProcessIntakeController.update`
- Observed behavior: route delegates logic to controller/service/model layers.
...
### Validations
- Joi middleware detected: `validate(updateAuditProcessIntakeSchema)`
- `company_name`: `Joi.string().max(255).allow(null`
- `company_tax_id`: `Joi.string().max(20).allow(null`
- `company_tax_address`: `Joi.string().max(255).allow(null`
- `company_sii_password`: `Joi.string().max(255).allow(null`
...
### Example Request
```http
PUT /api/company-audit-request/:companyId HTTP/1.1
Host: <api-host>
```
